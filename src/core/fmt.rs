...
impl<'a> module_codegen::CodeGenModule<'a> {
    ...
    fn generate_print_i32(&mut self) {
        let i32_type_point = self.context.i32_type().ptr_type(AddressSpace::Generic);
        let i32_type = self.context.i32_type();
        let empty_tuple = self.context.struct_type(&[], false);

        let fn_type = i32_type.fn_type(&[i32_type_point.into()], false);
        ...
        let val_alloca = self.builder.build_load(
            fn_addr.get_nth_param(0).unwrap().into_pointer_value(),
            "temp1",
        );
        ...
        self.builder
            .build_return(Some(&inkwell::values::BasicValueEnum::IntValue(
